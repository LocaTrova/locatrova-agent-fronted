...
export default function Navbar() {
  ...
  const linkBase =
    "inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-sm transition hover:bg-white/70";
  const active = (p: string) =>
    pathname === p
      ? "bg-white text-slate-900 ring-1 ring-white/50"
      : "text-slate-700";
    ...
    <header className="absolute inset-x-0 top-0 z-30">
      <div className="mx-auto w-full max-w-[2200px] px-4 sm:px-6 lg:px-8 pt-4">
        <div className="flex items-center justify-between rounded-full border border-white/40 bg-white/60 backdrop-blur-md px-3 sm:px-4 py-2 shadow-sm">
          ...
          <Link
            to="/"
            className="inline-flex items-center gap-2 text-slate-900 font-semibold text-sm sm:text-base"
          >
          ...
          >
            <Link to="/chat" className={`${linkBase} ${active("/chat")}`}>
              Chat
            </Link>
            <Link to="/apps" className={`${linkBase} ${active("/apps")}`}>
              App
            </Link>
            <Link
              to="/css-test"
              className={`${linkBase} ${active("/css-test")}`}
            >
              CSS
            </Link>
          </nav>
